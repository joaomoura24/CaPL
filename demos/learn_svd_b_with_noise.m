...
errb_svd = zeros(Ndatasets, Nnoise_levels);
%--------------------------------------------------------------------------
step = 0;
steps = Ndatasets;
...
for dataset_i = 1:Ndatasets
    ...
    parfor noise_i = 1:Nnoise_levels
        ...
        %------------------------------------------------------------------
        % compute MSE for b
        errb_svd(dataset_i, noise_i) = immse(abs(cell2mat(b_hat)),abs(b_true_vec));
        %------------------------------------------------------------------
        %% local
        % learn svd policy
