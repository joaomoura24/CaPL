...
upe_dpl_l = zeros(Ndatasets, Nnoise_levels);
upe_dpl_g = zeros(Ndatasets, Nnoise_levels);
cpe_dpl_l = zeros(Ndatasets, Nnoise_levels);
cpe_dpl_g = zeros(Ndatasets, Nnoise_levels);
%--------------------------------------------------------------------------
step = 0;
steps = Ndatasets;
...
for dataset_i = 1:Ndatasets
    ...
    parfor noise_i = 1:Nnoise_levels
        ...
        %% local
        % learn dpl policy
        model_dn.b = receptive_fields_weighted_regression_local(model_dn, S, Unoise, Phi);
        %------------------------------------------------------------------
        ...
        upe_dpl_l(dataset_i, noise_i) = e_dpl(Uu_eval, U_hat);
        cpe_dpl_l(dataset_i, noise_i) = e_dpl(Uns_eval, U_ns_hat);
        %------------------------------------------------------------------
        %% global
        % learn dpl policy 
        model_dn.b = receptive_fields_weighted_regression_global(model_dn, S, Unoise, Phi);
        %------------------------------------------------------------------
        % evaluate estimated policy
        pi_hat = def_weighted_linear_model_phi(model_dn);
        U_hat = cell2mat(cellfun(pi_hat, S_eval_cell, Phi_eval_cell, 'un',0));
        U_hat_cell = mat2cell(U_hat, u_dim, Ndatapoints*ones(Ntraj_eval,1));
        U_ns_hat = cell2mat(cellfun(@(P,v) P*v, P_eval, U_hat_cell, 'un', 0));
        upe_dpl_g(dataset_i, noise_i) = e_dpl(Uu_eval, U_hat);
        cpe_dpl_g(dataset_i, noise_i) = e_dpl(Uns_eval, U_ns_hat);
...
%% Save data to file
%--------------------------------------------------------------------------
save(['../data/data_errors_with_noise_dpl_', file_name], ...
    'upe_dpl_l','upe_dpl_g',...
    'cpe_dpl_l','cpe_dpl_g');
%--------------------------------------------------------------------------