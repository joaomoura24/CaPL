...
for dataset_i = 1:Ndatasets
    ...
    parfor traj_idx = 1:Ntraj_train
        ...
        %------------------------------------------------------------------
        % compute MSE for b
        errb_svd(dataset_i, traj_idx) = immse(abs(cell2mat(b_hat)),abs(b_true_vec_idx(1:traj_train(traj_idx))));
        %------------------------------------------------------------------
        % learn policy
...
save(['../data/data_errors_partial_svd_', file_name], 'traj_train',...
    'upe_svd_l','upe_svd_g',...
    'cpe_svd_l','cpe_svd_g',...
    'time_svd_l','time_svd_g', 'errb_svd');
%--------------------------------------------------------------------------